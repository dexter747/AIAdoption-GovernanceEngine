    ...
    <div className="p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        {statsCards.map(stat => (
          <div
            key={stat.name}
            className="rounded-xl p-6 border border-white/[0.06] bg-white/[0.02]"
          >
            <div className="flex items-center justify-between mb-4">
              <div
                className={`w-10 h-10 rounded-lg flex items-center justify-center ${stat.color === 'blue' ? 'bg-white/[0.05]' : stat.color === 'green' ? 'bg-white/[0.05]' : stat.color === 'purple' ? 'bg-white/[0.05]' : 'bg-white/[0.05]'}`}
              >
                <stat.icon
                  className={`w-5 h-5 ${
                    stat.color === 'blue'
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        ...
        <div className="rounded-xl p-6 border border-white/[0.06] bg-white/[0.02]">
          ...
          {chartData?.downloadChartData ? (
            <MiniBarChart
              data={chartData.downloadChartData}
              dataKey="downloads"
              color="bg-zinc-800"
            />
          ) : (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="rounded-xl border border-white/[0.06] bg-white/[0.02]">
          <div className="px-6 py-4 border-b flex items-center justify-between border-white/[0.06]">
            ...
            <a href="/dashboard/users" className="text-sm text-zinc-300 hover:text-zinc-400">
              View All
            </a>
          </div>
          <div className="p-4">
            {users.length === 0 ? (
              ...
              <div className="space-y-3">
                {users.map(user => (
                  <div
                    key={user.id}
                    className="flex items-center justify-between p-3 rounded-lg transition-colors hover:bg-white/[0.02]"
                  >
                    ...
                    <div className="text-right">
                      <span
                        className={`px-2 py-1 font-medium rounded-full ${user.plan === 'Enterprise' ? 'bg-white/[0.05] text-zinc-400' : user.plan === 'Pro' ? 'bg-white/[0.05] text-zinc-400' : 'bg-white/[0.04] text-zinc-500'}`}
                      >
                        {user.plan}
                      </span>
        ...
        <div className="rounded-xl border border-white/[0.06] bg-white/[0.02]">
          <div className="px-6 py-4 border-b flex items-center justify-between border-white/[0.06]">
            ...
            <a href="/dashboard/payments" className="text-sm text-zinc-300 hover:text-zinc-400">
              View All
            </a>
          </div>
          <div className="p-4">
              ...
              <div className="space-y-3">
                {payments.map(payment => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div
                        className={`w-9 h-9 rounded-full flex items-center justify-center ${payment.status === 'Completed' ? 'bg-white/[0.05]' : 'bg-white/[0.05]'}`}
                      >
                        <DollarSign
                          className={`w-4 h-4 ${
