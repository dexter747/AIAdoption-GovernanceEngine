...
export default function DashboardPage() {
 ...
 <span className="text-zinc-500">{item.value} ({percentage}%)</span>
 </div>
 ...
 <p className="text-zinc-500 mt-1">Overview of your platform analytics and metrics</p>
 </div>
 <button
 onClick={fetchData}
 className="flex items-center gap-2 px-4 py-2 text-sm text-zinc-500 rounded-lg transition-colors hover:text-white border-white/[0.06] hover:bg-white/[0.02]"
 >
 ...
 {/* Error Alert */}
 {error && (
 <div className="mb-6 border rounded-lg p-4 bg-zinc-950 border-white/[0.06]">
 ...
 {statsCards.map((stat) => (
 <div key={stat.name} className="rounded-xl p-6 border border-white/[0.06] bg-white/[0.02]">
 <div className="flex items-center justify-between mb-4">
 <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${ stat.color === 'blue' ? 'bg-white/[0.05]' : stat.color === 'green' ? 'bg-white/[0.05]' : stat.color === 'purple' ? 'bg-white/[0.05]' : 'bg-white/[0.05]' }`}>
 ...
 <div className="h-24 flex items-center justify-center text-zinc-500 text-sm">No data</div>
 )}
 </div>
 ...
 <div className="h-32 flex items-center justify-center text-zinc-500 text-sm">No users yet</div>
 )}
 </div>
 ...
 <div className="h-32 flex items-center justify-center text-zinc-500 text-sm">No downloads yet</div>
 )}
 </div>
 </div>
 ...
 <div className="px-6 py-4 border-b flex items-center justify-between border-white/[0.06]">
 ...
 <div className="space-y-3">
 {users.map((user) => (
 <div key={user.id} className="flex items-center justify-between p-3 rounded-lg transition-colors hover:bg-white/[0.02]">
 <div className="flex items-center gap-3">
 <div className="w-9 h-9 bg-gradient-to-br from-zinc-600 to-zinc-800 rounded-full flex items-center justify-center">
 ...
 <span className={`px-2 py-1 font-medium rounded-full ${ user.plan === 'Enterprise' ? 'bg-white/[0.05] text-zinc-400' : user.plan === 'Pro' ? 'bg-white/[0.05] text-zinc-400' : 'bg-white/[0.04] text-zinc-500' }`}>{user.plan}</span>
 ...
 <div key={payment.id} className="flex items-center justify-between p-3 rounded-lg transition-colors hover:bg-white/[0.02]">
 <div className="flex items-center gap-3">
 <div className={`w-9 h-9 rounded-full flex items-center justify-center ${ payment.status === 'Completed' ? 'bg-white/[0.05]' : 'bg-white/[0.05]' }`}>
 ...
 <p className="text-xs text-zinc-500">{payment.plan}</p>
 </div>
 </div>
 <div className="text-right">
