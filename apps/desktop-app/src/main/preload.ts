...
contextBridge.exposeInMainWorld('electron', {
  ...
  api: {
    ...
    testConnection: (connectionId: string) =>
      ipcRenderer.invoke('connection:test-by-id', connectionId),
    ...
    chat: (params: { messages: any[]; model: string; stream?: boolean }) =>
      ipcRenderer.invoke('ai:chat', params),
    queryWithMCP: (params: { connectionId: string; query: string; model: string }) =>
      ipcRenderer.invoke('mcp:query-with-ai', params),
    ...
    createCheckout: (params: { plan: string }) =>
      ipcRenderer.invoke('payments:create-checkout', params),
    ...
    updateUserPreferences: (preferences: any) =>
      ipcRenderer.invoke('user:update-preferences', preferences),
  ...
  mcp: {
    ...
    updateConnection: (id: string, updates: any) =>
      ipcRenderer.invoke('mcp:update-connection', id, updates),
    ...
    query: (connectionId: string, sql: string) =>
      ipcRenderer.invoke('mcp:query', connectionId, sql),
  ...
  chat: {
    ...
    getAllConversations: (options?: any) =>
      ipcRenderer.invoke('chat:get-all-conversations', options),
    updateConversation: (id: string, updates: any) =>
      ipcRenderer.invoke('chat:update-conversation', id, updates),
  ...
  express: {
    ...
    getUserApiKeyByProvider: (provider: string) =>
      ipcRenderer.invoke('express:get-user-api-key-by-provider', provider),
    ...
    updateUserApiKey: (keyId: string, updates: any) =>
      ipcRenderer.invoke('express:update-user-api-key', keyId, updates),
    ...
    getUserConnection: (connectionId: string) =>
      ipcRenderer.invoke('express:get-user-connection', connectionId),
    addUserConnection: (
      name: string,
    ...
    updateUserConnection: (connectionId: string, updates: any) =>
      ipcRenderer.invoke('express:update-user-connection', connectionId, updates),
    deleteUserConnection: (connectionId: string) =>
      ipcRenderer.invoke('express:delete-user-connection', connectionId),
    testUserConnection: (connectionId: string) =>
      ipcRenderer.invoke('express:test-user-connection', connectionId),
    startMCPServer: (connectionId: string) =>
      ipcRenderer.invoke('express:start-mcp-server', connectionId),

    // Usage & Subscriptions
    getUsage: (userId: string, options?: any) =>
      ipcRenderer.invoke('express:get-usage', userId, options),
    ...
    setAuth: (userId: string, licenseKey: string, authToken?: string) =>
      ipcRenderer.invoke('express:set-auth', userId, licenseKey, authToken),
  ...
  context: {
    ...
    importJson: (options?: { overwrite?: boolean }) =>
      ipcRenderer.invoke('context:import-json', options),
