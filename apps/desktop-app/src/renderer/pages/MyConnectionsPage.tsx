    ...
    <div className="h-full flex flex-col overflow-hidden">
      ...
      <div className="toolbar app-region-drag">
        ...
        <div className="flex items-center gap-2 app-region-no-drag">
          ...
          {connected > 0 && (
            <span className="text-[11px] text-emerald-500/70 tabular-nums">{connected} live</span>
          )}
          {errored > 0 && (
            <span className="text-[11px] text-red-500/60 tabular-nums">{errored} err</span>
          )}
          {queries > 0 && (
            <span className="text-[11px] text-white/30 tabular-nums">{queries} queries</span>
          )}
        </div>
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between">
                    <ConnectionIcon
                      logo={CONNECTION_LIBRARY[conn.type]?.logo}
                      icon={connInfo.icon}
                      color={connInfo.color}
                      bgColor={CONNECTION_LIBRARY[conn.type]?.bgColor ?? 'bg-gray-500/10'}
                      size="sm"
                    />
                  ...
                  <div>
                    ...
                    {conn.host && (
                      <p className="text-[10.5px] text-white/25 font-mono mt-0.5 truncate">
                        {conn.host}
                        {conn.port ? `:${conn.port}` : ''}
                      </p>
                    )}
