...
export default function LibraryPage() {
 ...
 {/* ── Category Tab Bar ─────────────────────────────────── */}
 <div className="flex items-center gap-1 px-3 py-2 border-b border-white/[0.055] overflow-x-auto flex-shrink-0 bg-[#0b0b0b]"
 ...
 <ConnectionIcon logo={conn.logo} icon={conn.icon} color={conn.color} bgColor={conn.bgColor} size="sm" />
 <div className="flex-1 min-w-0">
 <p className="text-[12px] font-medium text-white/80 leading-tight truncate">{conn.name}</p>
 <p className="text-[11px] text-white/35 leading-tight mt-0.5 line-clamp-2">{conn.description}</p>
 </div>
 <button
 onClick={() => isAdded ? navigate('/my-connections') : handleAddConnection(key)}
 className={cn(
 ...
 <div key={f.key}>
 <label className="block text-[11px] font-medium text-white/40 uppercase tracking-wider mb-1">{f.label}</label>
 <input
 type={f.type}
 placeholder={f.placeholder}
 value={(connectionConfig as any)[f.key]}
 onChange={(e) => setConnectionConfig({ ...connectionConfig, [f.key]: e.target.value })}
 ...
 placeholder={f.placeholder}
 value={(connectionConfig as any)[f.key]}
 onChange={(e) => setConnectionConfig({ ...connectionConfig, [f.key]: f.type === 'number' ? parseInt(e.target.value) : e.target.value })}
 className="input-desktop w-full"
 />
 </div>
 ))}
 ...
 >
 {isAdding ? <><Loader2 className="w-3 h-3 animate-spin" />Adding…</> : <><Plus className="w-3 h-3" />Add Connection</>}
 </button>
 </div>
 </motion.div>
 </motion.div>
