        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/[0.06]">
          <div>
            ...
            <p className="text-[12px] text-zinc-600 mt-0.5">
              Keys are AES-encrypted, stored locally
            </p>
          </div>
          <button
            onClick={onClose}
            className="w-7 h-7 flex items-center justify-center rounded-lg text-zinc-600 hover:text-zinc-300 hover:bg-white/[0.06] transition-all"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
          ...
          <div>
            ...
            {initialProvider ? (
              <div className="flex items-center gap-3 p-3 rounded-xl border border-white/[0.06] bg-white/[0.03]">
                ...
                {initialProvider.tag && (
                  <span className="ml-auto text-[10px] px-2 py-0.5 rounded-full border border-emerald-500/30 text-emerald-400">
                    {initialProvider.tag}
                  </span>
                )}
              </div>
              ...
              <div className="relative">
                <select
                  ...
                  onChange={e =>
                    setSelectedProvider(providers.find(p => p.id === e.target.value) ?? null)
                  }
...
export default function APIKeysPage() {
  ...
  const availableProviders = PROVIDERS.filter(p => !keys.some(k => k.provider === p.id));
    ...
    <div className="h-full flex flex-col overflow-hidden bg-[#0b0b0b]">
      ...
      <div className="toolbar app-region-drag">
        ...
        <span className="text-[11px] text-white/30 app-region-no-drag">
          BYOK — keys are AES-256 encrypted on-device
        </span>
        ...
        <div className="max-w-3xl">
          ...
          <div className="mb-6 flex items-center gap-3 px-4 py-3 rounded-xl border border-white/[0.05] bg-white/[0.02]">
            ...
            <p className="text-[12px] text-zinc-600">
              Your API keys are encrypted with AES-256 and stored only on this device. We never
              transmit or see them.
            </p>
          </div>
            ...
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-16 rounded-2xl border border-dashed border-white/[0.06] text-center">
                ...
                <p className="text-[12px] text-zinc-700">
                  Add your first key to use any AI provider
                </p>
              </div>
            ) : (
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {key.last_used_at && (
                          <p className="text-[11px] text-zinc-700 mt-0.5">
                            ...
                            {key.total_cost_cents > 0 &&
                              ` · $${(key.total_cost_cents / 100).toFixed(2)}`}
                          </p>
                        )}
                      </div>
              ...
              <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2">
                {availableProviders.map(provider => (
                  ...
                  >
                    ...
                    <p className="mt-2.5 text-[13px] font-medium text-zinc-300 leading-tight">
                      {provider.name}
                    </p>
                    {provider.tag && (
      ...
      {/* closes flex-1 overflow-auto p-5 */}
