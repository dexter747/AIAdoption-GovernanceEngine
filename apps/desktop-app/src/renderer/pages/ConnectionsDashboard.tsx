...
export default function ConnectionsDashboard() {
  ...
  const testConnection = async (connectionId: string) => {
    try {
      ...
      const result =
        (await window.electron.express?.testUserConnection?.(connectionId)) ||
      ...
      setConnections(prev =>
        prev.map(conn => (conn.id === connectionId ? { ...conn, status: 'error' } : conn))
      );
    } finally {
  ...
  const filteredTypes = Object.entries(CONNECTION_TYPES).filter(
    ([_key, value]) => selectedCategory === 'all' || value.category === selectedCategory
  );
      ...
      case 'error':
        return (
          <span className="flex items-center gap-1 text-[10px] font-medium text-red-400/70">
            <span className="w-1.5 h-1.5 bg-red-500/70 rounded-full" />
            Err
          </span>
        );
      default:
      ...
      <div className="toolbar app-region-drag">
        ...
        <div className="flex items-center gap-3 app-region-no-drag text-[11px] text-white/40">
          <span className="tabular-nums">
            <strong className="text-white/60 font-medium">{connections.length}</strong> total
          </span>
          {connections.filter(c => c.status === 'connected').length > 0 && (
            <span className="text-emerald-500/65 tabular-nums">
              {connections.filter(c => c.status === 'connected').length} live
            </span>
          )}
          {connections.filter(c => c.status === 'error').length > 0 && (
            <span className="text-red-400/60 tabular-nums">
              {connections.filter(c => c.status === 'error').length} err
            </span>
          )}
        </div>
        ...
        {CATEGORIES.map(cat => {
          ...
          return (
            <button
              ...
              className={cn(
                'tab-strip-item whitespace-nowrap flex items-center gap-1.5',
              ...
              {viewMode === 'grid' ? (
                <div className="grid gap-2.5 grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                  {filteredConnections.map(conn => {
                    const ct =
                      CONNECTION_TYPES[conn.type as keyof typeof CONNECTION_TYPES] ||
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="min-w-0">
                          <p className="text-[12.5px] font-medium text-white/80 truncate">
                            {conn.name}
                          </p>
                          <p className="text-[11px] text-white/35">{ct.name}</p>
                          {conn.host && (
                            <p className="text-[10.5px] text-white/22 font-mono mt-0.5 truncate">
                              {conn.host}
                              {conn.port ? `:${conn.port}` : ''}
                            </p>
                          )}
                ...
                <div className="rounded-lg border border-white/[0.07] overflow-hidden">
                  <div className="grid grid-cols-[28px_1fr_140px_80px_100px] gap-3 px-3 py-2 border-b border-white/[0.07] bg-[#0e0e0e]">
                    <div />
                    <span className="text-[10.5px] font-medium text-white/25 uppercase tracking-wider">
                      Name
                    </span>
                    <span className="text-[10.5px] font-medium text-white/25 uppercase tracking-wider">
                      Host
                    ...
                    <span className="text-[10.5px] font-medium text-white/25 uppercase tracking-wider text-right">
                      Actions
                    </span>
                  </div>
                  {filteredConnections.map((conn, i) => {
                    ...
                    return (
                      <div
                        key={conn.id}
                        className={cn(
                          'grid grid-cols-[28px_1fr_140px_80px_100px] items-center gap-3 px-3 py-2 border-b border-white/[0.04] transition-colors',
                          i % 2 === 0
                            ? 'bg-[#0d0d0d] hover:bg-[#101010]'
                            : 'bg-[#0b0b0b] hover:bg-[#0e0e0e]'
                        )}
                      >
                        <ConnectionIcon
                          logo={ct.logo}
          ...
          <div className="grid gap-2 grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-8">
            {filteredTypes.map(([key, config]) => (
              ...
              >
                ...
                />
                <p className="text-[11.5px] font-medium text-white/70 leading-tight">
                  {config.name}
                </p>
              </button>
            ))}
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
              <div className="w-full max-w-md rounded-xl border border-white/[0.10] bg-[#111] shadow-2xl p-6">
                ...
                <div className="space-y-3">
                  <div>
                    <label className="block text-[10.5px] font-medium text-white/40 uppercase tracking-wider mb-1">
                      Name
                    </label>
                    <input
                      type="text"
