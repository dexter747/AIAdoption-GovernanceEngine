...
export default function DashboardPage() {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      setConnections(
        connArray.map((c: any, idx: number) => ({
          id: idx + 1,
      ...
      try {
        ...
        setRecentQueries(
          Array.isArray(history)
            ? history.slice(0, 5).map((q: any, i: number) => ({
                id: i + 1,
          ...
          <div className="flex flex-col flex-1 min-h-0">
            <div className="flex items-center justify-between px-4 py-2 border-b border-white/[0.05]">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[11.5px] font-medium text-white/50 uppercase tracking-wider">
                  Recent Queries
                </span>
              </div>
              <Link
                to="/queries"
                className="text-[10.5px] text-white/30 hover:text-white/60 flex items-center gap-1 transition-colors"
              >
                View all
              ...
              {recentQueries.length === 0 ? (
                ...
                recentQueries.map((q, i) => (
                  <div
                    key={q.id}
                    className={cn(
                      'grid grid-cols-[1fr_80px_70px] items-center gap-3 px-4 py-2.5 border-b border-white/[0.04] transition-colors',
                      i % 2 === 0
            ...
            <div className="flex flex-wrap gap-2 p-4">
              {[
                ...
                <span
                  key={p.name}
                  className={cn(
                    'h-6 px-2.5 rounded-full text-[11px] font-medium border flex items-center gap-1.5 transition-all',
                    p.configured
            ...
            {connections.length === 0 ? (
              ...
              connections.map((db, i) => (
                <div
                  key={db.id}
                  className={cn(
                    'flex items-center gap-2.5 px-3 py-2 border-b border-white/[0.04] transition-colors',
                    i % 2 === 0
                      ? 'bg-[#0d0d0d] hover:bg-[#101010]'
                      : 'bg-[#0b0b0b] hover:bg-[#0e0e0e]'
                  )}
