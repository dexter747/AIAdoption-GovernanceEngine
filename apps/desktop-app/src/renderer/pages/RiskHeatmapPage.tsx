...
import { Globe, MapPin, AlertTriangle, Shield, ChevronRight } from 'lucide-react';
...
const MOCK_JURISDICTIONS: JurisdictionRisk[] = [
  {
    ...
    riskScore: 8,
    clientCount: 342,
    transactionVolume: 145000000,
    currency: 'GBP',
    flags: [],
    riskFactors: ['Home jurisdiction', 'MONEYVAL compliant'],
    lastReview: '2026-03-01',
    regulatoryStatus: 'JFSC regulated',
  ...
  },
  {
    ...
    currency: 'GBP',
    flags: [],
    riskFactors: ['Strong AML regime', 'FCA regulated corridor'],
  ...
  },
  {
    ...
    currency: 'CHF',
    flags: ['trust_structures'],
    riskFactors: ['Banking secrecy history', 'Trust opacity', 'Cross-border structuring risk'],
  ...
  },
  {
    ...
    riskScore: 72,
    clientCount: 12,
    transactionVolume: 18500000,
    currency: 'EUR',
    flags: ['maritime_trade', 'shell_companies'],
    riskFactors: [
      'Baltic corridor for Russian funds',
      'Maritime trade risk',
      'Shell company proliferation',
    ],
    lastReview: '2026-02-28',
    regulatoryStatus: 'FKTK regulated',
  ...
  },
  {
    ...
    riskScore: 78,
    clientCount: 8,
    transactionVolume: 12400000,
    currency: 'EUR',
    flags: ['sanctions_evasion', 'shell_companies'],
    riskFactors: [
      'Sanctions circumvention route',
      'Golden passport programme',
      'Russian capital flows',
    ],
    lastReview: '2026-02-25',
    regulatoryStatus: 'CySEC regulated',
  ...
  },
  {
    ...
    currency: 'USD',
    flags: ['sanctions', 'blocked'],
    riskFactors: [
      'Comprehensive sanctions regime',
      'OFSI/EU/OFAC designated',
      'All transactions blocked',
    ],
    lastReview: '2026-03-01',
    regulatoryStatus: 'Sanctioned',
    fatfStatus: 'FATF black list',
  },
  {
    id: 'j7',
    ...
    riskFactors: ['Trade-based ML risk', 'Free trade zone opacity', 'Gold trading corridor'],
    lastReview: '2026-02-20',
    regulatoryStatus: 'CBUAE regulated',
    fatfStatus: 'Grey list (removed 2024)',
  },
  {
    id: 'j8',
    ...
    riskScore: 68,
    clientCount: 45,
    transactionVolume: 42000000,
    currency: 'USD',
    flags: ['shell_companies', 'opaque_structures'],
    riskFactors: [
      'Offshore secrecy jurisdiction',
      'Limited beneficial ownership transparency',
      'Account takeover destination',
    ],
    lastReview: '2026-02-10',
    regulatoryStatus: 'FSC regulated',
    fatfStatus: 'Monitored',
  },
  {
    id: 'j9',
    ...
    riskScore: 15,
    clientCount: 156,
    transactionVolume: 198000000,
    currency: 'EUR',
    flags: [],
    riskFactors: ['Strong EU AML framework', 'Established Jersey-Normandy trade corridor'],
    lastReview: '2026-02-01',
    regulatoryStatus: 'AMF regulated',
    fatfStatus: 'Compliant',
  },
  {
    id: 'j10',
    ...
    riskScore: 10,
    clientCount: 98,
    transactionVolume: 87000000,
    currency: 'GBP',
    flags: [],
    riskFactors: ['Sister Crown Dependency', 'MONEYVAL compliant', 'Shared regulatory framework'],
    lastReview: '2026-01-15',
    regulatoryStatus: 'GFSC regulated',
    fatfStatus: 'Compliant',
  },
  {
    id: 'j11',
    ...
    currency: 'USD',
    flags: ['shell_companies', 'opacity'],
    riskFactors: [
      'Panama Papers jurisdiction',
      'Bearer share structures',
      'Limited CDD enforcement',
    ],
    lastReview: '2026-01-05',
    regulatoryStatus: 'SBP regulated',
    fatfStatus: 'Grey list',
  },
  {
    id: 'j12',
    ...
    riskScore: 42,
    clientCount: 28,
    transactionVolume: 56000000,
    currency: 'HKD',
    flags: [],
    riskFactors: [
      'China flow-through risk',
      'Trade-based ML corridor',
      'Cryptocurrency exchange hub',
    ],
    lastReview: '2026-02-12',
    regulatoryStatus: 'SFC regulated',
    fatfStatus: 'Compliant',
  },
  {
    id: 'j13',
    ...
    currency: 'SGD',
    flags: [],
    riskFactors: ['Strong AML regime', 'MAS regulatory oversight'],
    lastReview: '2026-02-18',
    regulatoryStatus: 'MAS regulated',
    fatfStatus: 'Compliant',
  },
  {
    id: 'j14',
    ...
    currency: 'USD',
    flags: ['fund_structures'],
    riskFactors: [
      'Offshore financial centre',
      'Fund domicile risk',
      'Improving transparency framework',
    ],
    lastReview: '2026-01-28',
    regulatoryStatus: 'CIMA regulated',
    fatfStatus: 'Compliant',
  },
  {
    id: 'j15',
    ...
    transactionVolume: 0,
    currency: 'USD',
    flags: ['sanctions', 'blocked', 'proliferation'],
    riskFactors: ['Comprehensive sanctions', 'FATF call for action', 'WMD proliferation risk'],
    lastReview: '2026-03-01',
    regulatoryStatus: 'Sanctioned',
    fatfStatus: 'FATF black list',
  },
  {
    ...
    currency: 'EUR',
    flags: [],
    riskFactors: ['Strong EU framework', 'Fund administration centre', 'CSSF regulated'],
    lastReview: '2026-02-05',
    regulatoryStatus: 'CSSF regulated',
    fatfStatus: 'Compliant',
  },
];
...
export default function RiskHeatmapPage() {
  ...
  useEffect(() => {
    ...
    const initMap = async () => {
      try {
        ...
        // Tile layer — dark or light depending on current theme
        const tileUrl =
          theme === 'light'
  ...
  useEffect(() => {
    ...
    const addMarkers = async () => {
      ...
      filtered.forEach(j => {
        const size =
          j.overallRisk === 'critical'
            ? 18
            : j.overallRisk === 'high'
              ? 14
              : j.overallRisk === 'medium'
                ? 11
                : 8;

        const markerBorder = theme === 'light' ? 'rgba(0,0,0,0.15)' : 'rgba(255,255,255,0.15)';
        ...
        // Tooltip — uses CSS variables so it flips with the theme
        marker.bindTooltip(
          `<div style="font-size:11px;font-weight:500;color:var(--text-primary)">${j.country}</div>
           <div style="font-size:10px;color:var(--text-secondary)">Risk Score: ${j.riskScore} · ${j.clientCount} clients</div>`,
    ...
    addMarkers();
  }, [mapReady, filter, filtered, theme]);
  ...
  const buildPDFConfig = (): PDFReportOptions => ({
    ...
    sections: [
      ...
      {
        type: 'table',
        title: 'Jurisdiction Risk Register',
        columns: ['Country', 'Code', 'Risk', 'Score', 'Clients', 'Volume', 'FATF Status'],
        rows: MOCK_JURISDICTIONS.sort((a, b) => b.riskScore - a.riskScore).map(j => [
          j.country,
          j.countryCode,
          j.overallRisk.toUpperCase(),
          String(j.riskScore),
          String(j.clientCount),
          fmtVolume(j.transactionVolume),
          j.fatfStatus || 'N/A',
        ]),
      },
      {
        ...
        content:
          'Risk scores are calculated using a composite model incorporating FATF mutual evaluation results, sanctions designations (OFSI/EU/UN/OFAC), Transparency International CPI scores, historical STR/SAR filing rates, and Jersey-specific corridor risk assessments per JFSC AML/CFT Handbook Chapter 4.',
      },
    ],
  });

  return (
    <div className="h-full flex flex-col overflow-hidden" style={{ background: 'var(--panel-bg)' }}>
      {/* Leaflet CSS overrides — use CSS variables so they flip with theme */}
      <style>{`
        .risk-tooltip { background: var(--panel-bg) !important; border: 1px solid var(--border-subtle) !important; border-radius: 8px !important; padding: 6px 10px !important; box-shadow: 0 4px 20px rgba(0,0,0,0.3) !important; }
        .risk-tooltip::before { border-top-color: var(--panel-bg) !important; }
        .leaflet-container { background: var(--panel-bg) !important; }
        .leaflet-control-zoom a { background: var(--surface-bg) !important; color: var(--text-secondary) !important; border-color: var(--border-subtle) !important; }
        .leaflet-control-zoom a:hover { background: var(--surface-raised) !important; color: var(--text-primary) !important; }
        ...
        <div className="flex items-center gap-1.5 app-region-no-drag">
          {(['all', 'critical', 'high', 'medium', 'low'] as const).map(f => (
            <button
              ...
              onClick={() => setFilter(f)}
              className={cn(
                'h-6 px-2.5 rounded text-[10px] font-medium capitalize transition-colors',
                filter === f
                  ? f === 'all'
            ...
            >
              ...
              {f !== 'all' && (
                <span className="ml-1 text-[9px] opacity-70">
                  ({MOCK_JURISDICTIONS.filter(j => j.overallRisk === f).length})
                </span>
              )}
            </button>
          ))}
      ...
      <div className="flex items-center gap-4 px-5 py-2.5 border-b border-white/[0.04]">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[11px] text-blue-400">
            {MOCK_JURISDICTIONS.filter(j => j.overallRisk === 'medium').length} Medium
          </span>
        </div>
        <div className="flex items-center gap-1.5">
          <div className="w-2 h-2 rounded-full bg-emerald-500" />
          <span className="text-[11px] text-emerald-400">
            {MOCK_JURISDICTIONS.filter(j => j.overallRisk === 'low').length} Low
          </span>
        </div>
        <div className="flex-1" />
        <span className="text-[10px] text-zinc-600">
          {stats.totalClients.toLocaleString()} clients · {fmtVolume(stats.totalVolume)} volume
            ...
            <div className="p-4 space-y-4">
              <div className="flex items-center justify-between">
                ...
                <span
                  className={cn(
                    ...
                    RISK_BG[selected.overallRisk]
                  )}
                >
                  {selected.overallRisk} risk
                </span>
              </div>
              <div>
                <h2 className="text-[16px] font-semibold text-white/90">{selected.country}</h2>
                <span className="text-[11px] text-zinc-500">
                  {selected.countryCode} · {selected.regulatoryStatus}
                </span>
              </div>
                ...
                <div className="h-1.5 bg-white/[0.06] rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      ...
                      background: RISK_DOT[selected.overallRisk],
                    }}
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <div className="p-2.5 rounded-lg bg-white/[0.02] border border-white/[0.04]">
                  ...
                  <div className="text-[14px] font-semibold text-white/80">
                    {fmtVolume(selected.transactionVolume)}
                  </div>
                </div>
                <div className="p-2.5 rounded-lg bg-white/[0.02] border border-white/[0.04]">
                  ...
                  <div
                    className={cn(
                      ...
                      selected.fatfStatus?.includes('black')
                        ? 'text-red-400'
                        : selected.fatfStatus?.includes('Grey')
                          ? 'text-amber-400'
                          : 'text-emerald-400'
                    )}
                  >
                    {selected.fatfStatus || 'N/A'}
                  </div>
                </div>
                <div className="p-2.5 rounded-lg bg-white/[0.02] border border-white/[0.04]">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {selected.flags.map((f, i) => (
                      <span
                        key={i}
                        className="text-[10px] px-1.5 py-0.5 rounded bg-red-900/20 text-red-400 border border-red-500/10 capitalize"
                      >
                        {f.replace(/_/g, ' ')}
                      </span>
                    ))}
              ...
              <div className="p-2.5 rounded bg-white/[0.02] border border-white/[0.03] text-[10px] text-zinc-500 leading-relaxed">
                ...
                Jurisdiction risk assessed per JFSC AML/CFT Handbook Chapter 4 — Risk-Based
                Approach.
                {selected.overallRisk === 'critical' &&
                  ' All transactions blocked per Sanctions and Asset-Freezing (Jersey) Law 2019.'}
                {selected.overallRisk === 'high' &&
                  ' Enhanced due diligence required for all client relationships and transactions.'}
              </div>
            </div>
          ) : (
            <div className="p-3">
                  ...
                  .map(j => (
                    ...
                    >
                      <div
                        ...
                        style={{ background: RISK_DOT[j.overallRisk] }}
                      />
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-[10px] text-zinc-600">
                          {j.clientCount} clients · {fmtVolume(j.transactionVolume)}
                        </div>
                      </div>
                      <div className="text-right">
                        <div
                          className={cn(
                            ...
                            RISK_CLR[j.overallRisk]
                          )}
                        >
                          {j.riskScore}
                        </div>
                        <div className={cn('text-[9px] capitalize', RISK_CLR[j.overallRisk])}>
                          {j.overallRisk}
                        </div>
                      </div>
