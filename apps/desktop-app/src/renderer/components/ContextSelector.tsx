...
export default function ContextSelector({
  ...
  const selectableContexts = contexts.filter(
    c => !c.autoInclude && c.connectionId !== connectionId && c.projectId !== projectId
  );

  // Filter by search
  const filteredContexts = searchQuery
    ? selectableContexts.filter(
        c =>
  ...
  return (
    <div className="border rounded-lg p-4">
      <div className="flex items-center justify-between mb-3">
        ...
        <span className="text-sm text-muted-foreground">{totalTokens.toLocaleString()} tokens</span>
      </div>
    ...
    <div className="flex flex-col max-h-96">
      ...
      <div className="overflow-y-auto flex-1">
        ...
        {showAutoIncluded && autoIncludedContexts.length > 0 && (
          <div className="p-2 border-b">
            <div className="text-xs font-medium text-muted-foreground mb-2">Auto-included</div>
            {autoIncludedContexts.map(ctx => (
              <ContextItem key={ctx.id} context={ctx} locked />
            ))}
          </div>
        )}
        ...
        {connectionContexts.length > 0 && (
          <div className="p-2 border-b">
            <div className="text-xs font-medium text-muted-foreground mb-2">Database Schema</div>
        ...
        {projectContexts.length > 0 && (
          <div className="p-2 border-b">
            <div className="text-xs font-medium text-muted-foreground mb-2">Project Context</div>
        ...
        {selectableContexts.length > 0 && (
          <div className="p-2">
            <div className="text-xs font-medium text-muted-foreground mb-2">Available Contexts</div>
        ...
        {selectableContexts.length === 0 && autoIncludedContexts.length === 0 && (
          <div className="p-4 text-center text-sm text-muted-foreground">No contexts available</div>
        )}
      </div>
    ...
    const compile = async () => {
      ...
      try {
        ...
        const configs: Record<
          string,
          { maxTokens: number; reservedForResponse: number; reservedForConversation: number }
        > = {
          'gpt-4o': {
            maxTokens: 128000,
