...
router.get('/', validateJwt, async (req, res, next) => {
  try {
    ...
    const { data, error } = await supabase
      ...
      .select('id, provider, key_name, key_preview, config, is_active, is_valid, last_used_at, total_requests, total_tokens, total_cost_cents, created_at')
...
router.post('/', validateJwt, async (req, res, next) => {
  try {
    ...
    if (existing) {
      throw new ApiError(409, `Key "${validated.key_name}" already exists for ${PROVIDER_INFO[validated.provider].name}`);
    }
...
router.post('/:id/test', validateJwt, async (req, res, next) => {
  try {
    ...
    logger.info({ userId: req.user.id, keyId: id, provider: keyData.provider }, 'API key validated successfully');
