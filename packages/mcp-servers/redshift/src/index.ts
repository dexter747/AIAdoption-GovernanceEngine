...
function initConnection(): void {
  if (!process.env.REDSHIFT_HOST) console.error('Warning: REDSHIFT_HOST not set');
  if (!process.env.REDSHIFT_PORT) console.error('Warning: REDSHIFT_PORT not set');
  if (!process.env.REDSHIFT_DATABASE) console.error('Warning: REDSHIFT_DATABASE not set');
  if (!process.env.REDSHIFT_USER) console.error('Warning: REDSHIFT_USER not set');
...
async function main(): Promise<void> {
  ...
  const server = new Server(
    { name: 'redshift-mcp-server', version: '1.0.0' },
    { capabilities: { tools: {} } }
  );
  ...
  console.error('Amazon Redshift MCP server running on stdio');
}
