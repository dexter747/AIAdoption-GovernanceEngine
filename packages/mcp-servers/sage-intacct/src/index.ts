...
const TOOLS: Tool[] = [
  {
    name: 'read_objects',
    description: 'Read objects by query',
    inputSchema: {
      type: 'object',
      properties: {
        object: { type: 'string', description: 'Object type (e.g., CUSTOMER, VENDOR, GLACCOUNT)' },
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: { object: { type: 'string' }, key: { type: 'string' } },
      required: ['object', 'key'],
    },
  },
  {
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: { object: { type: 'string' }, key: { type: 'string' }, data: { type: 'object' } },
      required: ['object', 'key', 'data'],
    },
  },
  {
    name: 'run_report',
    ...
    inputSchema: {
      type: 'object',
      properties: { reportName: { type: 'string' }, params: { type: 'object' } },
      required: ['reportName'],
    },
  },
  {
...
const server = new Server(
  { name: 'sage-intacct-mcp-server', version: '1.0.0' },
  { capabilities: { tools: {} } }
);
...
<operation><authentication><login><userid>${process.env.INTACCT_USER_ID}</userid><companyid>${process.env.INTACCT_COMPANY_ID}</companyid><password>${process.env.INTACCT_USER_PASSWORD}</password></login></authentication><content><function controlid="login"><getAPISession /></function></content></operation></request>`;
  ...
  const r = await axios.post(endpoint, loginXml, {
  ...
  try {
    switch (name) {
      case 'read_objects': {
        ...
        const r = await axios.post(endpoint, buildXml(xml), {
          headers: { 'Content-Type': 'application/xml' },
        });
      ...
      case 'create_object': {
        const dataXml = Object.entries((args as any).data)
          .map(([k, v]) => `<${k}>${v}</${k}>`)
          .join('');
    ...
    return {
      content: [{ type: 'text' as const, text: `Sage Intacct Error: ${error.message}` }],
      isError: true,
    };
  }
});
