...
function initConnection(): void {
  if (!process.env.WOOCOMMERCE_URL) console.error('Warning: WOOCOMMERCE_URL not set');
  if (!process.env.WOOCOMMERCE_CONSUMER_KEY)
    console.error('Warning: WOOCOMMERCE_CONSUMER_KEY not set');
...
async function main(): Promise<void> {
  ...
  const server = new Server(
    { name: 'woocommerce-mcp-server', version: '1.0.0' },
    { capabilities: { tools: {} } }
  );
