...
function initConnection(): void {
  if (!process.env.AZURE_SQL_HOST) console.error('Warning: AZURE_SQL_HOST not set');
  if (!process.env.AZURE_SQL_DATABASE) console.error('Warning: AZURE_SQL_DATABASE not set');
  if (!process.env.AZURE_SQL_USER) console.error('Warning: AZURE_SQL_USER not set');
...
async function main(): Promise<void> {
  ...
  const server = new Server(
    { name: 'azure-sql-mcp-server', version: '1.0.0' },
    { capabilities: { tools: {} } }
  );
