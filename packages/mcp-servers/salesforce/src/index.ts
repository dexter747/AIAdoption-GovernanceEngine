...
async function querySalesforce(soql: string) {
  ...
  try {
    ...
    return { content: [{ type: 'text' as const, text: `Salesforce Error: ${error.message}` }], isError: true };
  }
}
...
async function searchRecords(searchTerm: string) {
  ...
  try {
    ...
    return { content: [{ type: 'text' as const, text: JSON.stringify(result.searchRecords, null, 2) }] };
  } catch (error: any) {
    return { content: [{ type: 'text' as const, text: `Error: ${error.message}` }], isError: true };
  }
}
...
async function getRecentRecords(objectName: string, limit: number = 10) {
  if (!conn) throw new Error('Not connected');
  try {
    const result = await conn.sobject(objectName)
      .select('*')
...
server.setRequestHandler(ListResourcesRequestSchema, async () => ({
  resources: [
    { uri: 'salesforce://accounts', name: 'Recent Accounts', description: 'Recently modified accounts', mimeType: 'application/json' },
    { uri: 'salesforce://contacts', name: 'Recent Contacts', description: 'Recently modified contacts', mimeType: 'application/json' },
    { uri: 'salesforce://opportunities', name: 'Open Opportunities', description: 'Open sales opportunities', mimeType: 'application/json' },
    { uri: 'salesforce://leads', name: 'Recent Leads', description: 'Recent leads', mimeType: 'application/json' },
  ],
}));
...
server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
  ...
  if (uri === 'salesforce://accounts') {
    result = await conn.query('SELECT Id, Name, Industry, AnnualRevenue, Website FROM Account ORDER BY LastModifiedDate DESC LIMIT 50');
  } else if (uri === 'salesforce://contacts') {
    result = await conn.query('SELECT Id, Name, Email, Phone, Account.Name FROM Contact ORDER BY LastModifiedDate DESC LIMIT 50');
  } else if (uri === 'salesforce://opportunities') {
    result = await conn.query("SELECT Id, Name, Amount, StageName, CloseDate, Account.Name FROM Opportunity WHERE IsClosed = false ORDER BY CloseDate ASC LIMIT 50");
  } else if (uri === 'salesforce://leads') {
    result = await conn.query('SELECT Id, Name, Email, Company, Status FROM Lead ORDER BY CreatedDate DESC LIMIT 50');
  } else {
    throw new Error(`Unknown resource: ${uri}`);
  }
  
  return { contents: [{ uri, mimeType: 'application/json', text: JSON.stringify(result.records, null, 2) }] };
});
  ...
  tools: [
    {
      ...
      inputSchema: { type: 'object', properties: { soql: { type: 'string', description: 'SOQL query' } }, required: ['soql'] },
    },
    {
      name: 'search',
    ...
    },
    {
      ...
      inputSchema: { type: 'object', properties: { object: { type: 'string', description: 'Object API name' } }, required: ['object'] },
    },
    {
      name: 'create_record',
