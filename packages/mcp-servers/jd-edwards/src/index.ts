...
const TOOLS: Tool[] = [
  {
    name: 'call_orchestration',
    description: 'Call a JDE orchestration',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'data_request',
    description: 'Execute a data request',
    inputSchema: {
      type: 'object',
      properties: {
        table: { type: 'string', description: 'Table name (e.g., F0101)' },
  ...
  {
    name: 'form_request',
    description: 'Submit a form service request',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'batch_formrequest',
    description: 'Submit batch form requests',
    inputSchema: {
      type: 'object',
  ...
  },
  {
    ...
    description: 'Get media objects for a record',
    inputSchema: {
      type: 'object',
      properties: { moStructure: { type: 'string' }, moKey: { type: 'string' } },
      required: ['moStructure', 'moKey'],
    },
  },
];

const server = new Server(
  { name: 'jd-edwards-mcp-server', version: '1.0.0' },
  { capabilities: { tools: {} } }
);
...
async function initConnection() {
  ...
  const r = await axios.post(`${baseUrl}/tokenrequest`, {
    username,
    password,
    environment,
    deviceName: 'VelanovaMCP',
  });
  ...
  try {
    switch (name) {
      case 'call_orchestration': {
        const r = await api.post('/orchestrator', {
          name: (args as any).name,
          inputs: (args as any).inputs || {},
        });
      ...
      case 'data_request': {
        const body: any = {
          ...
          maxRecords: (args as any).maxRecords || 50,
        };
        if ((args as any).fields)
          body.outputFields = (args as any).fields.map((f: string) => ({ field: f }));
      ...
      case 'form_request': {
        const body = {
          formName: (args as any).form,
          applicationName: (args as any).application,
          action: (args as any).action || 'read',
          formServiceAction: (args as any).action || 'R',
    ...
    return {
      content: [
        {
          type: 'text' as const,
          text: `JDE Error: ${error.response?.data?.message || error.message}`,
        },
      ],
      isError: true,
    };
