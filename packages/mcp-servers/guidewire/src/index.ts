...
const TOOLS: Tool[] = [
  {
    ...
    description: 'Search insurance policies (PolicyCenter)',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'search_claims',
    description: 'Search claims (ClaimCenter)',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description: 'Search billing accounts (BillingCenter)',
    inputSchema: {
      type: 'object',
      properties: { accountNumber: { type: 'string' }, accountName: { type: 'string' } },
    },
  },
  {
    name: 'get_account',
  ...
  {
    name: 'cloud_api',
    description: 'Generic Guidewire Cloud API call',
    inputSchema: {
      type: 'object',
      properties: {
        center: { type: 'string', enum: ['pc', 'cc', 'bc'] },
        endpoint: { type: 'string' },
        method: { type: 'string', enum: ['GET', 'POST', 'PATCH'] },
        data: { type: 'object' },
      },
      required: ['center', 'endpoint'],
    },
  },
];

const server = new Server(
  { name: 'guidewire-mcp-server', version: '1.0.0' },
  { capabilities: { tools: {} } }
);

async function initConnection() {
  const headers = {
    Authorization: `Bearer ${process.env.GUIDEWIRE_ACCESS_TOKEN}`,
    'Content-Type': 'application/json',
  };
  if (process.env.GUIDEWIRE_PC_URL)
    apiPC = axios.create({ baseURL: `${process.env.GUIDEWIRE_PC_URL}/rest/common/v1`, headers });
  if (process.env.GUIDEWIRE_CC_URL)
    apiCC = axios.create({ baseURL: `${process.env.GUIDEWIRE_CC_URL}/rest/common/v1`, headers });
  if (process.env.GUIDEWIRE_BC_URL)
    apiBC = axios.create({ baseURL: `${process.env.GUIDEWIRE_BC_URL}/rest/common/v1`, headers });
  ...
  try {
    switch (name) {
      ...
      case 'cloud_api': {
        ...
        const r = await (api as any)[method](
          ...
          method !== 'get' ? (args as any).data : { params: (args as any).data }
        );
        return { content: [{ type: 'text' as const, text: JSON.stringify(r.data, null, 2) }] };
      }
      default:
    ...
    return {
      content: [
        {
          type: 'text' as const,
          text: `Guidewire Error: ${error.response?.data?.userMessage || error.message}`,
        },
      ],
      isError: true,
    };
