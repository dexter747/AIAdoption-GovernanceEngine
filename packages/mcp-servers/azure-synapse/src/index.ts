...
function initConnection(): void {
  if (!process.env.SYNAPSE_HOST) console.error('Warning: SYNAPSE_HOST not set');
  if (!process.env.SYNAPSE_DATABASE) console.error('Warning: SYNAPSE_DATABASE not set');
  if (!process.env.SYNAPSE_USER) console.error('Warning: SYNAPSE_USER not set');
...
async function main(): Promise<void> {
  ...
  const server = new Server(
    { name: 'azure-synapse-mcp-server', version: '1.0.0' },
    { capabilities: { tools: {} } }
  );
  ...
  console.error('Azure Synapse Analytics MCP server running on stdio');
}
