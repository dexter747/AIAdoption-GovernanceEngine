...
const TOOLS: Tool[] = [
  {
    ...
    inputSchema: {
      type: 'object',
      properties: { policyNumber: { type: 'string' }, filter: { type: 'string' } },
    },
  },
  {
    name: 'get_policy',
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: { policyId: { type: 'string' }, status: { type: 'string' } },
    },
  },
  {
    name: 'get_claim',
  ...
  {
    name: 'api_call',
    description: 'Generic Duck Creek API call',
    inputSchema: {
      type: 'object',
      properties: {
...
const server = new Server(
  { name: 'duck-creek-mcp-server', version: '1.0.0' },
  { capabilities: { tools: {} } }
);
...
server.setRequestHandler(CallToolRequestSchema, async request => {
  ...
  try {
    switch (name) {
      ...
      case 'get_billing': {
        const r = await api.get(`/billing/accounts/${(args as any).accountId}`);
        return { content: [{ type: 'text' as const, text: JSON.stringify(r.data, null, 2) }] };
      }
      case 'api_call': {
    ...
    return {
      content: [{ type: 'text' as const, text: `Duck Creek Error: ${error.message}` }],
      isError: true,
    };
  }
});
