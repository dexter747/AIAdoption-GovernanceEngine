  ...
  try {
    ...
    for (const p of priorities) {
      const result = await makeRequest(
        `table/incident?sysparm_query=priority=${p}&sysparm_count=true`
      );
    ...
    for (const state of states) {
      const result = await makeRequest(
        `table/incident?sysparm_query=state=${state}&sysparm_count=true`
      );
...
server.setRequestHandler(ListResourcesRequestSchema, async () => ({
  resources: [
    {
      uri: 'servicenow://incidents/open',
      name: 'Open Incidents',
      description: 'All open incidents',
      mimeType: 'application/json',
    },
    {
      uri: 'servicenow://incidents/critical',
      name: 'Critical Incidents',
      description: 'P1 and P2 incidents',
      mimeType: 'application/json',
    },
    {
    ...
    {
      uri: 'servicenow://problems/open',
      name: 'Open Problems',
      description: 'Open problem records',
      mimeType: 'application/json',
    },
  ],
}));
...
server.setRequestHandler(ReadResourceRequestSchema, async request => {
  ...
  if (uri === 'servicenow://incidents/open') {
    ...
    data = await makeRequest(
      'table/incident?sysparm_query=priority<=2^active=true&sysparm_limit=50'
    );
  } else if (uri === 'servicenow://changes/pending') {
    data = await makeRequest(
      'table/change_request?sysparm_query=approval=requested&sysparm_limit=50'
    );
  ...
  return {
    contents: [{ uri, mimeType: 'application/json', text: JSON.stringify(data.result, null, 2) }],
  };
});
