...
const TOOLS: Tool[] = [
  {
    name: 'query',
    description: 'Execute a CQL query on Cassandra',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: { keyspace: { type: 'string', description: 'Keyspace name' } },
      required: ['keyspace'],
    },
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: { keyspace: { type: 'string' }, table: { type: 'string' } },
...
const server = new Server(
  { name: 'cassandra-mcp-server', version: '1.0.0' },
  { capabilities: { tools: {} } }
);
...
server.setRequestHandler(CallToolRequestSchema, async request => {
  ...
  try {
    switch (name) {
      case 'query': {
        const result = await client.execute((args as any).cql, (args as any).params, {
          prepare: true,
        });
        return {
          content: [
            {
              type: 'text' as const,
              text: JSON.stringify(
                { rows: result.rows, columns: result.columns?.map(c => c.name) },
                null,
                2
              ),
            },
      ...
      case 'describe_table': {
        const result = await client.execute(
          ...
          [(args as any).keyspace, (args as any).table]
        );
        return { content: [{ type: 'text' as const, text: JSON.stringify(result.rows, null, 2) }] };
      }
      default:
