  ...
  try {
    ...
    return {
      content: [
        {
          type: 'text' as const,
          text: JSON.stringify({ key: data.key, self: data.self }, null, 2),
        },
      ],
    };
  } catch (error: any) {
...
async function getTransitions(key: string) {
  try {
    ...
    return {
      content: [{ type: 'text' as const, text: JSON.stringify(data.transitions, null, 2) }],
    };
  } catch (error: any) {
    return { content: [{ type: 'text' as const, text: `Error: ${error.message}` }], isError: true };
  }
...
server.setRequestHandler(ListResourcesRequestSchema, async () => ({
  resources: [
    ...
    },
    {
      uri: 'jira://sprints',
      name: 'Active Sprints',
      description: 'Currently active sprints',
      mimeType: 'application/json',
    },
  ],
}));
...
server.setRequestHandler(ReadResourceRequestSchema, async request => {
  ...
  if (uri === 'jira://myissues') {
    data = await makeRequest(
      'search?jql=assignee=currentUser() AND resolution=Unresolved ORDER BY updated DESC&maxResults=50'
    );
    ...
    contents: [
      {
        uri,
        mimeType: 'application/json',
        text: JSON.stringify(data.issues || data.values, null, 2),
      },
    ],
  };
});
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          transitionId: {
            type: 'string',
            description: 'Transition ID (use get_transitions to find)',
          },
        },
