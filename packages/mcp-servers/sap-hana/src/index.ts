...
function executeQuery(sql: string, params: any[] = []): Promise<any> {
  ...
  return new Promise((resolve, reject) => {
    connection.exec(sql, params, (err: any, rows: any) => {
      if (err) {
        resolve({ content: [{ type: 'text' as const, text: `SAP HANA Error: ${err.message}` }], isError: true });
      } else {
  ...
  return new Promise((resolve) => {
    connection.exec(explainSql, [], (err: any) => {
      ...
      connection.exec(
        ...
        (err2: any, rows: any) => {
          if (err2) {
            resolve({ content: [{ type: 'text' as const, text: `Error: ${err2.message}` }], isError: true });
          } else {
            resolve({ content: [{ type: 'text' as const, text: JSON.stringify(rows, null, 2) }] });
          }
        }
...
server.setRequestHandler(ListResourcesRequestSchema, async () => ({
  resources: [
    { uri: 'hana://tables', name: 'All Tables', description: 'List of all tables in current schema', mimeType: 'application/json' },
    { uri: 'hana://views', name: 'All Views', description: 'List of all views', mimeType: 'application/json' },
    { uri: 'hana://procedures', name: 'Stored Procedures', description: 'List of stored procedures', mimeType: 'application/json' },
    { uri: 'hana://calculation-views', name: 'Calculation Views', description: 'SAP HANA Calculation Views', mimeType: 'application/json' },
  ],
}));
...
server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
  ...
  if (uri === 'hana://tables') {
    sql = 'SELECT TABLE_NAME, RECORD_COUNT, TABLE_TYPE FROM SYS.TABLES WHERE SCHEMA_NAME = CURRENT_SCHEMA ORDER BY TABLE_NAME';
  } else if (uri === 'hana://views') {
    sql = 'SELECT VIEW_NAME, VIEW_TYPE FROM SYS.VIEWS WHERE SCHEMA_NAME = CURRENT_SCHEMA ORDER BY VIEW_NAME';
  } else if (uri === 'hana://procedures') {
    sql = "SELECT PROCEDURE_NAME, INPUT_PARAMETER_COUNT, OUTPUT_PARAMETER_COUNT FROM SYS.PROCEDURES WHERE SCHEMA_NAME = CURRENT_SCHEMA ORDER BY PROCEDURE_NAME";
  } else if (uri === 'hana://calculation-views') {
    sql = "SELECT PACKAGE_ID, OBJECT_NAME, OBJECT_SUFFIX FROM _SYS_REPO.ACTIVE_OBJECT WHERE OBJECT_SUFFIX = 'calculationview' ORDER BY OBJECT_NAME";
  } else {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  switch (name) {
    ...
    case 'list_tables':
      return executeQuery('SELECT TABLE_NAME, RECORD_COUNT FROM SYS.TABLES WHERE SCHEMA_NAME = CURRENT_SCHEMA ORDER BY TABLE_NAME');
    ...
    case 'get_active_connections':
      return executeQuery('SELECT CONNECTION_ID, CONNECTION_STATUS, USER_NAME, CLIENT_HOST FROM SYS.M_CONNECTIONS WHERE CONNECTION_STATUS = \'RUNNING\'');
