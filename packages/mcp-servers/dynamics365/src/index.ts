...
const TOOLS: Tool[] = [
  {
    name: 'query_entities',
    description: 'Query Dynamics 365 entities using OData',
    inputSchema: {
      type: 'object',
      properties: {
        entity: { type: 'string', description: 'Entity set name (e.g., accounts, contacts)' },
        filter: { type: 'string', description: 'OData $filter expression' },
        select: { type: 'string', description: 'Comma-separated fields to select' },
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: { entity: { type: 'string' }, id: { type: 'string' }, data: { type: 'object' } },
  ...
  {
    name: 'get_metadata',
    description: 'Get entity metadata/definitions',
    inputSchema: {
      type: 'object',
      properties: { entity: { type: 'string', description: 'Entity logical name (optional)' } },
    },
  },
  {
    name: 'execute_fetchxml',
    description: 'Execute a FetchXML query',
    inputSchema: {
      type: 'object',
...
const server = new Server(
  { name: 'dynamics365-mcp-server', version: '1.0.0' },
  { capabilities: { tools: {} } }
);
...
server.setRequestHandler(CallToolRequestSchema, async request => {
  ...
  try {
    switch (name) {
      case 'query_entities': {
        ...
        return {
          content: [
            {
              type: 'text' as const,
              text: JSON.stringify({ count: r.data.value?.length, records: r.data.value }, null, 2),
            },
          ],
        };
      }
      ...
      case 'create_record': {
        ...
        return {
          content: [
            {
              type: 'text' as const,
              text: JSON.stringify({ success: true, id: r.headers['odata-entityid'] }, null, 2),
            },
          ],
        };
      }
      ...
      case 'get_metadata': {
        ...
        const url = entity
          ...
          : '/EntityDefinitions?$select=LogicalName,DisplayName,EntitySetName';
      ...
      case 'execute_fetchxml': {
        ...
        return {
          content: [{ type: 'text' as const, text: JSON.stringify(r.data.value, null, 2) }],
        };
      }
      default:
    ...
    return {
      content: [{ type: 'text' as const, text: `Dynamics 365 Error: ${msg}` }],
      isError: true,
    };
  }
});
